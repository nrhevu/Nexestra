/**
 * The scripted core of the fake harness.
 *
 * A `HarnessAdapter` whose behaviour for one run is a `FakeRunScript`: some
 * files to write into the worktree, some events to emit, and how slowly to
 * emit them. Everything else in this package — the named scenarios in
 * `scenarios.ts`, the adapter in `adapter.ts` — is a function that produces
 * one of these.
 *
...
export interface FakeRunScript {
  ...
  events?: HarnessEvent[];
  /**
   * Emitted instead of `events` when the run has to react to something —
   * a permission answer, a cancel — rather than replay a fixed list.
   */
  stream?: (context: FakeStreamContext) => AsyncIterable<HarnessEvent>;
...
/** What a `stream` script is handed: the run, plus the levers a live run has. */
export interface FakeStreamContext extends FakeRunContext {
  /** Aborts when the caller aborts *or* when `control(cancel)` arrives. */
  signal: AbortSignal;
  /** True once either signal has aborted. */
  cancelled(): boolean;
  /** Sleep, resolving early when the run is cancelled. */
  sleep(ms: number): Promise<void>;
  /**
   * Block until `control(runId, {action:"answer_permission", requestId})`
   * arrives. Resolves `undefined` when the run is cancelled first.
   */
  awaitPermission(requestId: string): Promise<boolean | undefined>;
  /** Write files into the worktree mid-run, as a real harness would. */
...
export interface ScriptedFakeAdapterOptions {
  id?: HarnessId;
...
/** Kept as the public name of the options bag for `createFakeHarnessAdapter`. */
export type FakeAdapterOptions = ScriptedFakeAdapterOptions;
...
/** Restart the `run_fake…` id sequence; only tests that assert on ids need it. */
export function resetFakeRunIds(): void {
  counter = 0;
}

/**
 * A `HarnessAdapter` that replays scripts. Prefer `createFakeAdapter()` unless
 * you want to spell out the exact event list a run emits.
 */
export function createFakeHarnessAdapter(
  options: ScriptedFakeAdapterOptions = {},
): FakeHarnessAdapter {
  ...
  const permissions = new Map<string, (approved: boolean) => void>();
  let maxConcurrent = 0;
  ...
  const adapter = {
    ...
    async *run(prepared: PreparedRun, signal: AbortSignal): AsyncIterable<HarnessEvent> {
      ...
      const context: FakeRunContext = { spec, prepared, attempt, index: calls.length };
      const resolved = options.script?.(context);
      ...
      const isCancelled = () => signal.aborted || controller.signal.aborted;
      ...
      try {
        ...
        await writeFiles(script.files ?? {});

        if (script.stream) {
          const streamContext: FakeStreamContext = {
            ...context,
            signal: controller.signal,
            cancelled: isCancelled,
            sleep: (ms) => sleepUntil(ms, signal, controller.signal),
            awaitPermission: (requestId) =>
              awaitPermission(permissions, answers, requestId, signal, controller.signal),
            writeFiles,
          };
          for await (const event of script.stream(streamContext)) {
    ...
    async control(runId: string, action: RunControl): Promise<void> {
      ...
      if (action.action === "answer_permission") {
        answers.set(action.requestId, action.approved);
        permissions.get(action.requestId)?.(action.approved);
        permissions.delete(action.requestId);
      }
    },
...
/** Sleep, but wake up early when one of the signals aborts. */
function sleepUntil(ms: number, ...signals: AbortSignal[]): Promise<void> {
  return new Promise((resolve) => {
...
/**
 * Wait for `control(answer_permission)`.
 *
 * The answer can arrive *before* the run gets round to waiting for it: a
 * caller that reacts to the `permission_request` event synchronously answers
 * while the generator is still suspended on the `yield`. So an answer already
 * recorded wins, and only a genuinely unanswered request blocks.
 */
function awaitPermission(
  registry: Map<string, (approved: boolean) => void>,
  answered: ReadonlyMap<string, boolean>,
  requestId: string,
  ...signals: AbortSignal[]
): Promise<boolean | undefined> {
  const already = answered.get(requestId);
  ...
  return new Promise((resolve) => {
    ...
    registry.set(requestId, (approved) => {
